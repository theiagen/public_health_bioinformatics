...
workflow theiameta_panel_illumina_pe {
  ...
  call gather_scatter_task.gather_scatter {
    input:
      samplename = samplename,
      taxon_ids = write_lines(taxon_ids),
      organism = write_lines(krakentools.organism_name),
      extracted_read1 = write_lines(krakentools.extracted_read1),
      extracted_read2 = write_lines(krakentools.extracted_read2),
      krakentools_docker = write_lines(krakentools.krakentools_docker),
      fastq_scan_num_reads_binned1 = write_lines(fastq_scan_binned.read1_seq),
      fastq_scan_num_reads_binned2 = write_lines(fastq_scan_binned.read2_seq),
      fastq_scan_num_reads_binned_pairs = write_lines(fastq_scan_binned.read_pairs),
      fastq_scan_docker = write_lines(fastq_scan_binned.fastq_scan_docker),
      fastq_scan_version = write_lines(fastq_scan_binned.version),
      pilon_assembly_fasta = write_lines(pilon.assembly_fasta), # maybe??
      quast_genome_length = write_lines(quast.genome_length),
      quast_number_contigs = write_lines(quast.number_contigs),
      quast_n50 = write_lines(quast.n50_value),
      quast_gc_percent = write_lines(quast.gc_percent),
      number_N = write_lines(morgana_magic.number_N),
      number_ATCG = write_lines(morgana_magic.number_ATCG),
      number_Degenerate = write_lines(morgana_magic.number_Degenerate),
      number_Total = write_lines(morgana_magic.number_Total),
      percent_reference_coverage = write_lines(morgana_magic.percent_reference_coverage),
      pango_lineage = write_lines(morgana_magic.pango_lineage),
      pango_lineage_expanded = write_lines(morgana_magic.pango_lineage_expanded),
      pangolin_conflicts = write_lines(morgana_magic.pangolin_conflicts),
      pangolin_notes = write_lines(morgana_magic.pangolin_notes),
      pangolin_assignment_version = write_lines(morgana_magic.pangolin_assignment_version),
      pangolin_versions = write_lines(morgana_magic.pangolin_versions),
      pangolin_docker = write_lines(morgana_magic.pangolin_docker),
      nextclade_version = write_lines(morgana_magic.nextclade_version),
      nextclade_docker = write_lines(morgana_magic.nextclade_docker),
      nextclade_ds_tag = write_lines(morgana_magic.nextclade_ds_tag),
      nextclade_aa_subs = write_lines(morgana_magic.nextclade_aa_subs),
      nextclade_aa_dels = write_lines(morgana_magic.nextclade_aa_dels),
      nextclade_clade = write_lines(morgana_magic.nextclade_clade),
      nextclade_lineage = write_lines(morgana_magic.nextclade_lineage),
      nextclade_qc = write_lines(morgana_magic.nextclade_qc),
      nextclade_ds_tag_flu_ha = write_lines(morgana_magic.nextclade_ds_tag_flu_ha),
      nextclade_aa_subs_flu_ha = write_lines(morgana_magic.nextclade_aa_subs_flu_ha),
      nextclade_aa_dels_flu_ha = write_lines(morgana_magic.nextclade_aa_dels_flu_ha),
      nextclade_clade_flu_ha = write_lines(morgana_magic.nextclade_clade_flu_ha),
      nextclade_qc_flu_ha = write_lines(morgana_magic.nextclade_qc_flu_ha),
      nextclade_ds_tag_flu_na = write_lines(morgana_magic.nextclade_ds_tag_flu_na),
      nextclade_aa_subs_flu_na = write_lines(morgana_magic.nextclade_aa_subs_flu_na),
      nextclade_aa_dels_flu_na = write_lines(morgana_magic.nextclade_aa_dels_flu_na),
      nextclade_clade_flu_na = write_lines(morgana_magic.nextclade_clade_flu_na),
      nextclade_qc_flu_na = write_lines(morgana_magic.nextclade_qc_flu_na)
  } 
  output {
    # kraken2 outputs
