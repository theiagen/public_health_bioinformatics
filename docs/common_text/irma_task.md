...
??? task "`IRMA`: Influenza Assembly and Characterization"
    Cleaned reads are assembled using IRMA which stands for Iterative Refinement Meta-Assembler. IRMA first sorts reads to Flu genome segments using LABEL, then iteratively maps reads to a collection of reference sequences (in this case for Influenza virus) and iteratively edits the references to account for high population diversity and mutational rates that are characteristic of Influenza genomes. Assemblies produced by IRMA will be ordered from largest to smallest assembled flu segment. IRMA also performs typing and subtyping as part of the assembly process. Note: IRMA does not differentiate between Flu B Victoria and Yamagata lineages. For determining this information, please review the ABRicate task outputs which will provide this information.
...
<!-- if: assembly -->
    ...
    - `assembly_fasta` - The full genome assembly in FASTA format, with 1 FASTA entry per genome segment. There should be 8 segments in total, but depending on the quality and depth of sequence data, some segments may not be assembled nor present in this output file.
