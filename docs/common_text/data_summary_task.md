...
<!-- endif -->
    ...
    If you fill out the `data_summary_*` and `sample_names` optional inputs, you can use the optional `summarize_data` task. The task takes a comma-separated list of column names from the Terra data table. The task checks whether the contents of the columns are shared between the samples and creates a CSV file of the results.
    ...
    The CSV file indicates presence (TRUE) or absence (empty) for each item. By default, the task does not add a Phandango coloring tag to group items from the same column, but you can turn this on by setting `phandango_coloring` to `true`.

    ??? toggle "Example use of Phandango coloring"
        Data summary produced using the `phandango_coloring` option, visualized alongside a Newick tree at <http://jameshadfield.github.io/phandango/#/main>
