...
workflow read_QC_trim_pe {
  ...
  if (("~{workflow_series}" == "theiacov") || ("~{workflow_series}" == "theiameta")) {
    ...
    if (read_scrubber == "hostile"){
      call hostile_task.hostile_pe {
        input:
          samplename = samplename,
          read1 = read1_raw,
          read2 = read2_raw
  ...
  if ("~{workflow_series}" == "theiacov") {
    ...
    call kraken.kraken2_theiacov as kraken2_theiacov_dehosted {
      input:
        samplename = samplename,
        read1 = select_first([ncbi_scrub_pe.read1_dehosted, hostile_pe.read1_dehosted]),
        read2 = select_first([ncbi_scrub_pe.read2_dehosted, hostile_pe.read2_dehosted]),
        target_org = target_org
    }
  }
  if (read_processing == "trimmomatic"){
    call trimmomatic.trimmomatic_pe {
      input:
        samplename = samplename,
        read1 = select_first([ncbi_scrub_pe.read1_dehosted, hostile_pe.read1_dehosted, read1_raw]),
        read2 = select_first([ncbi_scrub_pe.read2_dehosted, hostile_pe.read2_dehosted, read2_raw]),
  ...
  output {
    # NCBI scrubber / hostile
    File? read1_dehosted = select_first([hostile_pe.read1_dehosted, ncbi_scrub_pe.read1_dehosted])
    File? read2_dehosted = select_first([hostile_pe.read2_dehosted, ncbi_scrub_pe.read2_dehosted])
    ...
    String? hostile_docker = hostile_pe.hostile_docker
