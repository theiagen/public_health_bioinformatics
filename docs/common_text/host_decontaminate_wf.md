...
??? task "`read_decontaminate`: Mapping-based Read Decontamination"
    ...
    Known contaminate genetic data can be removed by mapping directly to an inputted `contaminate_fasta`. This input can be a host genome, common microbial contaminate genome, or intentionally spiked sequences. The mapping statistics and aligned reads to the contaminate FASTA are outputted, while downstream quality control tasks will input the decontaminated reads.
