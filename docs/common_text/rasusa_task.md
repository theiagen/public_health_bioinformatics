...
fragment: true
---
<!-- if: ont|read_qc_trim|theiaviral -->
??? task "`Rasusa`: Read Subsampling (optional)"
<!-- endif -->
<!-- if: wf -->
...
<!-- if: ont -->
    By default, this task will subsample TheiaProk_ONT reads to a depth of 150X using an estimated genome length of 5 million basepairs and TheiaEuk_ONT reads using an estimated genome length of 50 million basepairs. The estimated genome length can be changed by the user by providing a different value for the `genome_length` input parameter. The target subsampling depth can also be adjusted by modifying the `rasusa_downsampling_coverage` variable.

    !!! dna "Activate Rasusa by providing a downsampling metric"
        One of the following mutually exclusive parameters must be set to enable Rasusa downsampling:

        - `genome_length`
        - `rasusa_num_bases`
        - `rasusa_fraction_of_reads`
        - `rasusa_num_reads`
<!-- endif -->
    ...
    | `--coverage` + `--genome-size` | Default mode. Subsamples to a target depth using a genome length. |
...
<!-- if: read_qc_trim -->
    ...
    When running in coverage mode, it's strongly recommended to manually set the `rasusa_genome_length` input parameter in order to ensure accurate downsampling. If not provided, `rasusa_genome_length` falls back to `raw_check_reads.est_genome_length` from the `read_screen` task. Oftentimes, this value can overestimate the true genome length, particularly for large, high-coverage FASTQ files. If the screen on the raw reads is skipped (`skip_screen_raw` in TheiaCoV, `skip_screen` in TheiaProk and TheiaEuk), no genome length is estimated, so you must supply `genome_length` explicitly or use one of the override modes above.
<!-- endif -->

<!-- if: theiaviral -->
    The Rasusa task performs subsampling on the input raw reads and is disabled by default. Users can enable it by setting the `skip_rasusa` variable to `false`. When activated, it subsamples reads to a target depth of 250X, using the estimated genome length either generated by the `datasets_genome_length` task or provided directly by the user. The target subsampling depth can also be adjusted by modifying the `coverage` variable.
