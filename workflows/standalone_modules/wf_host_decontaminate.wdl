...
workflow host_decontaminate {
  ...
  if (is_accession) {
    call ncbi_datasets.identify as download_accession {
      input: 
        ncbi_accession = host
    }
  }
  if (identify.accession == "NA") {
    String identify_status = "FAIL"
  }
  ...
  output {
    ...
    String ncbi_datasets_status = select_first([identify_status, download_taxon.ncbi_datasets_status, download_accession.ncbi_datasets_status])
