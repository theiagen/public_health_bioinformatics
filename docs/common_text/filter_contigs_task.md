...
<!-- if: flye -->
    This task filters the created contigs based on a user-defined minimum length threshold (default of 1000) and removes homopolymer contigs (contigs of any length that consist of a single nucleotide).
<!-- endif -->
<!-- if: digger -->
    ...
    Options are available to skip any of these filters by setting the respective parameters to `false`: 
        ...
        - `filter_contigs_skip_coverage_filter`, and
...
<!-- endif -->
    ...
    !!! techdetails "Filter Contigs Technical Details" 
        |  | Links |
        | --- | --- |
        | Task | [task_filter_contigs.wdl](https://github.com/theiagen/public_health_bioinformatics/blob/main/tasks/quality_control/read_filtering/task_filter_contigs.wdl) |
