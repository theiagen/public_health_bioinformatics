...
??? task "`Flye`: _De novo_ Assembly"
    ...
    ??? toggle "`Polypolish`: Hybrid Assembly Polishing ==_for ONT and Illumina data_=="
        Polishes assemblies with hybrid ONT and Illumina data using `task_polypolish.wdl`. Polypolish is a short-read polishing tool that uses Illumina reads to correct errors in long-read assemblies.
    ...
    ??? toggle "`Medaka`: Polishing of Flye assembly"
        Polishes the Flye assembly using `task_medaka.wdl`. Medaka uses the raw reads to polish the assembly and generate a consensus sequence.
