...
task ts_mlst {
  ...
  command <<< 
    ...
    if [[ ~{run_secondary_scheme}]]; then
      scheme_list=$(mlst --list)
      ...
      if grep -q "$secondary_scheme" "$scheme_list"; then
        mlst \
          ...
          ~{assembly} \
          >> ~{samplename}_ts_mlst_${secondary_scheme}.tsv
              # parse ts mlst tsv for relevant outputs

        # if output TSV only contains one line (header line); no ST predicted
        if [ $(wc -l ~{samplename}_ts_mlst${secondary_scheme}.tsv | awk '{ print $1 }') -eq 1 ]; then
          predicted_mlst_secondary="No ST predicted"
          pubmlst_scheme_secondary="NA"
        # else, TSV has more than one line, so parse outputs
        else
          pubmlst_scheme_secondary="$(cut -f2 ~{samplename}_ts_mlst_${secondary_scheme}.tsv | tail -n 1)"
          predicted_mlst_secondary="ST$(cut -f3 ~{samplename}_ts_mlst_${secondary_scheme}.tsv | tail -n 1)"
          # allelic_profile: take second line of output TSV; cut to take 4th column and beyond; replace tabs with commas
          allelic_profile_secondary="$(cut -f 4- ~{samplename}_ts_mlst_${secondary_scheme}.tsv | tail -n 1 | sed -e 's|\t|,|g')"
          ...
          awk 'NR>1' ~{samplename}_ts_mlst_${secondary_scheme}.tsv > ~{samplename}_ts_mlst_${secondary_scheme}_no_header.tsv
        fi
    ...
    if [ -f "~{samplename}_ts_mlst_${secondary_scheme}.tsv" ]; then
      cat ~{samplename}_ts_mlst_${secondary_scheme}_no_header.tsv >> ~{samplename}_ts_mlst.tsv
    fi
  ...
  output {
    ...
    String? ts_mlst_predicted_secondary_st = read_string("PUBMLST_SECONDARY_ST")
    ...
    File? ts_mlst_secondary_novel_alleles = "~{samplename}_novel_mlst_alleles_${secondary_scheme}.fasta"
